...
float3 FresnelLerpCustom (float3 x, float3 y, float d)
{
...
//light calculations
    ...
    ////Gs = F(NdotL) * F(NdotV);
     ...
     float3 unityIndirectSpecularity =  indirectSpecular * FresnelLerpCustom(specColor,grazingTerm,NdotV) * max(0.15,_Metallic) * (1-roughness*roughness* roughness);
