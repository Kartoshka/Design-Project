...
namespace MadGoat_SSAA
{
    ...
    public class MadGoatSSAA_Editor : Editor
    {
        ...
        private SerializedProperty exposeInternalRender;
        //private SerializedProperty flipImageFix;
        ...
        protected GUIStyle normal_style = new GUIStyle();
        protected GUIStyle accent_style = new GUIStyle();
        protected GUIStyle category_style = new GUIStyle();

        private void SetupStyles()
        {
            accent_style = new GUIStyle("ShurikenModuleTitle");
            accent_style.normal.textColor = accent_pro; //Application.HasProLicense() ? accent_pro : accent_basic;
            accent_style.fontSize = 16;
            accent_style.fontStyle = FontStyle.Normal;
            accent_style.fixedHeight = 40;

            normal_style.normal.textColor = Application.HasProLicense() ? normal_pro : normal_basic;
            normal_style.fontSize = 12;

            // get style from unity's particles
            category_style = new GUIStyle("ShurikenModuleTitle")
            {
                fontStyle = FontStyle.Bold,
                fontSize = 12,
                fixedHeight = 30,
            };
            category_style.normal.textColor = accent_pro;
        }
        private void DrawHeader(string message)
        {
            GUI.backgroundColor = new Color(.4f, .4f, .4f, 1);
            GUILayout.Label(message, category_style);
        ...
        public void OnEnable()
        {
            ...
            exposeInternalRender = serObj.FindProperty("exposeInternalRender");

           // flipImageFix = serObj.FindProperty("flipImageFix");
        ...
        public virtual void DrawTab2() // Screenshot Tab 
        {
            ...
            if (screenshotTab == 0)
            {
                ...
                accent_style.fontSize = 12;
                DrawHeader("*Render Resolution: " + (target as MadGoatSSAA).screenshotSettings.outputResolution * (target as MadGoatSSAA).screenshotSettings.screenshotMultiplier);
            ...
            else
            {
                ...
                DrawHeader("*Render Resolution: " + panoramaSettings.FindPropertyRelative("panoramaSize").intValue* panoramaSettings.FindPropertyRelative("panoramaMultiplier").intValue + " x " + panoramaSettings.FindPropertyRelative("panoramaSize").intValue * panoramaSettings.FindPropertyRelative("panoramaMultiplier").intValue + " x 6 faces");
        ...
        public virtual void DrawTab3() // General Tab 
        {
            ...
            EditorGUILayout.Separator();
            EditorGUILayout.Separator();
            EditorGUILayout.HelpBox("Exposes the internal render camera in the hierarchy for advanced uses such as offline rendering and adding components to it", MessageType.Info);
            exposeInternalRender.boolValue = EditorGUILayout.Toggle("Expose Internal Renderer", exposeInternalRender.boolValue);
